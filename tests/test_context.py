    ...
    def test_is_sensitive_path_true(self):
        ...
        with patch('pathlib.Path.expanduser') as mock_expand:
            with patch('pathlib.Path.resolve') as mock_resolve:
                ...
                mock_expand.return_value.resolve.return_value.relative_to = Mock(return_value=Path('keys'))
        ...
        with patch('ais.core.context.run_safe_command') as mock_run:
            with patch('pathlib.Path.cwd') as mock_cwd:
                with patch('pathlib.Path.iterdir') as mock_iterdir:
                    ...
                    mock_cwd.return_value.iterdir.return_value = [mock_file1, mock_file2]
...
class TestCollectContext:
        ...
        with patch('pathlib.Path.cwd', return_value=Path('/home/user/project')):
            with patch('ais.core.context.is_sensitive_path', return_value=False):
                with patch('ais.core.context.collect_core_context') as mock_core:
                    mock_core.return_value = {'command': 'test', 'cwd': '/home/user/project'}
    ...
    def test_collect_context_no_config(self):
        ...
        with patch('ais.core.context.get_config') as mock_get_config:
            mock_get_config.return_value = {'context_level': 'minimal', 'sensitive_dirs': []}
