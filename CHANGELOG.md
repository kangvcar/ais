...
- 🚀 **优化ais setup用户体验**：实施方案3+4混合方案，提供智能交互式配置
  - **方案3-改进提示信息**: 显示清晰的三种配置生效方式选择
  - **方案4-智能询问用户**: 交互式询问是否立即在当前终端生效配置
  - 用户可选择：立即生效、新终端生效、或临时测试
  - 自动source失败时提供明确的手动操作指导
  - 支持交互式和非交互式环境，提供友好的降级提示
  - 透明化配置过程，用户完全了解每个步骤和选择
